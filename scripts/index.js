...
const fbEmail = process.env.fbEmail
const fbPassword = process.env.fbPassword
...
async function diplayCommits() {
  try {
    ...
    const tinder_client = await createClientFromFacebookLogin({
      emailAddress: fbEmail,
      password: fbPassword,
    })

    const parameters = {
      location: '大阪', // swipeしたい場所を入力(有料会員のみ)
      score: 50,
      firstMessage: 'こんにちは:D',
